...
Scope {
    ...
    readonly property color colDefaultBlue: Theme.accent
    ...
    function iconFromName(name) {
        ...
        return Globals.themedIcon(s, Theme.appIconTheme, "app")
            || Globals.themedIcon(s, Theme.iconTheme, "mono");
    }
    ...
    function resolveAppIcon(stream) {
        ...
        function push(v) {
            ...
            if (!s)
                return;
            // Electron apps all advertise as Chromium to Pulse — that mark
            // is a last resort, never the first hit.
            if (s.toLowerCase() === "chromium")
                return;
            cands.push(s);
            cands.push(s.toLowerCase());
        }
        push(stream.icon_name);
        if (stream.app_id)
            push(String(stream.app_id).split(".").pop());
        if (stream.binary) {
            let b = String(stream.binary).replace(/^\./, "").replace(/-wrapped$/, "");
            push(b);
        }
        if (stream.name)
            push(String(stream.name).split(" ")[0]);
        push(stream.application);
    ...
    PanelWindow {
        ...
        // Rise from the volume pill, but clamp so a 420px panel never runs
        // off the right edge when the pill sits in the tray cluster.
        readonly property int panelW: 420
        readonly property int screenW: {
            const s = Globals.shellScreen
            return (s && s.width) ? s.width : 2560
        }
        readonly property int desiredLeft: Math.round(
            Globals.volPillX + Globals.volPillWidth / 2 - panelW / 2
        )
        readonly property int maxLeft: Math.max(Theme.pad, screenW - panelW - Theme.pad)
        anchors {
            bottom: true
            left: true
        }
        margins {
            bottom: 2
            left: Math.max(Theme.pad, Math.min(desiredLeft, maxLeft))
        }
                    ...
                    Row {
                        ...
                        Repeater {
                            ...
                            Item {
                                ...
                                readonly property bool selected: mixerWin.tab === index
                                ...
                                MfShape {
                                    anchors.fill: parent
                                    kind: "sliceTL"
                                    slant: Theme.tabSlice - 2
                                    fillColor: mixTab.selected ? root.colCardHover : Theme.hatch
                                    strokeColor: mixTab.selected ? Theme.accentHot : Theme.hairline
                                    strokeWidth: mixTab.selected ? Theme.strokeActive : Theme.hairlineWidth
                                ...
                                Text {
                                    ...
                                    text: mixTab.modelData
                                    color: mixTab.selected ? root.colFg : root.colMuted
                                    font.family: Theme.fontFamily
                                    font.pixelSize: Theme.fontSizeSm
                                    font.bold: mixTab.selected
                                }

                                MouseArea {
                                    anchors.fill: parent
                                    cursorShape: Qt.PointingHandCursor
                                    onClicked: mixerWin.tab = mixTab.index
                                }
                            }
                        }
                    }
    ...
    component ChipBtn: Rectangle {
        ...
        implicitHeight: 20
        radius: 0
        color: selected ? Qt.rgba(chip.accent.r, chip.accent.g, chip.accent.b, 0.22) : Theme.hatch
        border.color: selected ? Qt.rgba(chip.accent.r, chip.accent.g, chip.accent.b, 0.55) : Theme.hairline
        border.width: 1
