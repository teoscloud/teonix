...
in {
  ...
  # Keep the real .conf installed: if it is missing, Hyprland writes a stub
  # with autogenerated=1 (yellow banner). Next login uses hyprland.lua via
  # `Hyprland -c`. Live reloads still look at .conf first.
  home.file.".config/hypr/hyprland.conf".source = "${dotfilesPath}/config/hypr/hyprland.conf";
  home.file.".config/hypr/hyprland.lua".source = "${dotfilesPath}/config/hypr/hyprland.lua";
  ...
  home.file.".config/hypr/hyprpaper.conf".text = ''
    ...
    wallpaper {
        monitor =
        path = ${config.home.homeDirectory}/.config/hypr/wallpapers/brancastle.jpg
        fit_mode = cover
    }
  '';
  ...
  # Hyprland writes a root-owned stub (autogenerated = 1) if .conf is missing.
  # That stub is what shows the yellow banner. Delete only that stub — never
  # a real config — so a live reload cannot fall back to autogenerated.
  home.activation.removeAutogeneratedHyprlandConf = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    conf="$HOME/.config/hypr/hyprland.conf"
    if [ -f "$conf" ] && grep -q '^autogenerated = 1' "$conf"; then
      rm -f "$conf" || chmod u+w "$conf" 2>/dev/null; rm -f "$conf" || true
    fi
  '';
