...
in
{
  home.file.".zshaliases.sh".source = pkgs.writeText ".zshaliases.sh" ''
    ...
    stablenixupgrade() {
      ...
      cd "${projectdir}" && _nix_nice sudo nixos-rebuild switch --flake "path:."#${hostname} ${impureFlag} "''${_nix_rebuild_opts[@]}"
    }
    stableupdatehome() {
      _nix_stable_copy_lock || return
      cd "${projectdir}" && _nix_nice home-manager switch -b bak --flake "path:."#${hostname} ${impureFlag}
    }
    stablesystemupdate() {
      _nix_stable_copy_lock || return
      cd "${projectdir}" && _nix_nice sudo nixos-rebuild switch --flake "path:."#${hostname} ${impureFlag} "''${_nix_rebuild_opts[@]}" && _nix_nice home-manager switch -b bak --flake "path:."#${hostname} ${impureFlag}
    }
