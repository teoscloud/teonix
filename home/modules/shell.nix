...

{
  ...
  home.file.".zshaliases.sh".source = pkgs.writeText ".zshaliases.sh" ''
    alias systemupdate='cd ${projectdir} && nix flake update && sudo nixos-rebuild switch --flake "path:."#${hostname} --impure && home-manager switch --flake "path:."#${username}'
    alias updatehome='cd ${projectdir} && home-manager switch --flake "path:."#${username}'
