...
1. adds a swapfile sized from this Mac's RAM — 16 GiB on an 8 GiB Mac — and switches zram off so those pages reach the disk instead of RAM (needed if the Asahi kernel has to compile)
...
There is no public binary cache for `linux-asahi` ([upstream docs](https://github.com/nix-community/nixos-apple-silicon/blob/main/docs/binary-cache.md)). `#applenix` asks for the latest apple-silicon **release** kernel (`packages.aarch64-linux.linux-asahi`), built from that input's own pinned nixpkgs so routine `systemupdate`s do not change the hash.

Nix reuses the store path if it is already there (official ISO matching the pin in [`flake.nix`](../../flake.nix)). If it is not, the Mac compiles it — hours, which is why stage 2 sizes swap and caps `--option cores`. No matching, no refusal.
...
**`Out of memory: Killed process … (nix)` / `builder … died with signal SIGKILL` during stage 2.** Usually the Asahi kernel compiling (see [The Asahi kernel](#the-asahi-kernel)). Three mitigations are in place:
